		...
		<div className='flex'>
			<div className="w-full md:w-1/2 flex flex-col items-center p-6 md:p-12">
				<form onSubmit={onHandleSubmit} className="w-full max-w-sm bg-white p-4 md:p-8 rounded-lg border shadow-sm">
					...
					<button type='submit' className='w-full bg-black text-white p-2 rounded-lg font-semibold hover:bg-gray-800 cursor-pointer transition'>
						{ loading ? "Loading..." : "Sign Up"}
					</button>
					<p className="mt-6 text-center text-sm">
						Don't have an account? <Link to={`/login?redirect=${encodeURIComponent(redirect)}`} className='text-blue-500'>Login</Link>
					</p>
				</form>
			</div>
			...
			<div className="hidden md:block w-1/2">
				<div className="h-full flex flex-col justify-center items-center">
					<img src={register} alt="login_img" className='h-[660px] w-full object-cover' />
				</div>
			</div>
		</div>
	)
