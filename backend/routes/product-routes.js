...
router.post("/", protect, admin, async (req, res) => {
	try {
		...
		console.log(error)
		res.status(500).send("Error while product");
	}
});
...
router.put("/:id", protect, admin, async (req, res) => {
	try {
		...
		if (product) {
			product.name = name || product.name; // update name, (if not provided then remain old value)
...
router.get("/", async (req, res) => {
	try {
		...
		// console.log(gender, brand, search)
		...
		console.error(error);
		res.status(500).json({ message: "Failed to Filtering the products" });
	}
});
...
router.get("/best-seller", async (req, res) => {
	try {
		...
		console.log(error)
		res.status(500).json({message: "Failed to find best seller"});
	}
});
...
router.get("/new-arrivals", async (req, res) => {
	try {
		...
		console.log(error);
		res.status(500).json({message: "Failed to find new arrivals"});
	}
});
...
router.get("/:id", async (req, res) => {
	try {
		...
		console.log(error);
		res.status(500).json({ message: "Failed to find a single Product" });
	}
});
